...
{
    if (n < 0)
        throw std::runtime_error("factorial: negative argument");
    if (n == 0)
        return 1;
...
public:
    ...
    double calculate() const { return val; }
...
{
    auto calc_v = [](const auto& x) { return x->calculate(); };
    return std::visit(calc_v, expr);
}

double Bin_expr::calculate() const
{
    switch (op) {
        ...
        case '*': return left.calculate() * right.calculate();
        case '/': return left.calculate() / right.calculate();
        case '%': return std::fmod(left.calculate(), right.calculate());
        case '!' : return factorial(left.calculate());
...
Expression primary(Token_stream& ts)
{
    ...
    switch (t.kind) {
        ...
        case '+':    // "+" Primary
            return primary(ts);
        case '!':    // "!" Primary
            return Expression{'!', primary(ts), Expression{0}};
        default:
...
Expression term(Token_stream& ts)
{
    ...
    for (;;) {
        ...
        case '%':    // Term "%" Primary
            ...
            break;
        case '!':    // Term Primary "!"
            left = Expression('!', left, Expression{0});
            t = ts.get();
            break;
        default:    // Primary
